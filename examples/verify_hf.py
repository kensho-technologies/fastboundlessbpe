...
"""Verify a HuggingFace export matches our tokenizer over a corpus.
...
Usage:
    python examples/verify_hf.py --model-file model.model --dataset data.jsonl \
...
def main() -> None:
    ...
    total_tokens = 0
    extra_supermerges = 0
    applied_supermerges = 0
    for text in docs:
        ours = [tok.vocab.id_to_token[i] for i in tok.encode_ordinary(text)]
        theirs = [tobytes(hf.id_to_token(i)) for i in hf.encode(text).ids]
    ...
    print(f"Checked {len(docs)} docs against HuggingFace export:")
