...
# Train second-pass SuperBPE model
# This loads the first-pass word model and trains ONLY supermerges (no competition)
# Final model will have vocab_size word operations + additional supermerges
#
# Usage: python -u train_super.py --num-lines 10000 --supermerges 53180 2>&1 | tee log_super_10k.txt
#
# The supermerge count should match the number of supermerges from the corresponding BoundlessBPE run
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.fast:
        from boundlessbpe import FastSuperBpeTrainer
        assert FastSuperBpeTrainer is not None, "Rust extension not available - run 'maturin develop --release' first"
        tokenizer = FastSuperBpeTrainer(
            script_specific_regex=ss_regex,
            script_specific_scripts=ss_scripts,
        )
    else:
        from boundlessbpe.train import SuperBpeTrainer
        tokenizer = SuperBpeTrainer(pretokenizer)

    # Print training parameters
    print("num_lines:", args.num_lines)
    print("vocab_size:", args.vocab_size)
    print("supermerges:", args.supermerges)
    ...
    # SuperBPE second pass: no deletions (tau=1.1)
    tau = 1.1
    outprefix = f"{args.output_prefix}_{args.num_lines}_{args.max_bytes}_{args.supermerges}_{tau}_{int(args.blowup)}_{impl}_superbpe"
    print("outprefix:", outprefix)

    # Train SuperBPE (second pass, supermerges only)
    tokenizer.train(args.filepath, outprefix, args.num_lines, args.supermerges, args.recalc, word_model_file, args.max_bytes,
    ...
    print("saving unified SuperBPE model")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train second-pass SuperBPE model")
    parser.add_argument("--num-lines", type=int, required=True, help="Number of documents to read from training file")
    parser.add_argument("--supermerges", type=int, required=True, help="Number of supermerges to create (should match BoundlessBPE run)")
...
# Example commands:
# python -u train_super.py --num-lines 10000 --supermerges 53180 2>&1 | tee log_super_10k.txt
# python -u train_super.py --num-lines 10000 --supermerges 53180 --fast 2>&1 | tee log_super_fast_10k.txt
# python -u train_super.py --num-lines 10000 --supermerges 53180 --vocab-size 40960 2>&1 | tee log_super_10k_40k.txt
# python -u train_super.py --num-lines 100000 --supermerges 53180 --word-model ./models/my_word_model.model 2>&1 | tee log_super_100k.txt
# python -u train_super.py --num-lines 1000000 --supermerges 53180 --filepath data/other.jsonl 2>&1 | tee log_super_1M.txt
